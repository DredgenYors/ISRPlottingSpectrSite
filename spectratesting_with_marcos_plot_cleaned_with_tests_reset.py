...
def classify_thomson_hard(alpha, k_lambdaD=None):
    ...
    Thomson / non-collective: kλD > 1   (alpha < 1)
    Collective:              kλD < 1   (alpha > 1)
    Boundary:                kλD == 1  (alpha == 1)
    """
    if k_lambdaD is None:
        k_lambdaD = 1.0 / alpha

    # Use a tiny tolerance for floating error around 1
    eps = 1e-12
...
def print_case_header(label, alpha_back, k_lambdaD):
    ...
    print("Regime:", classify_thomson_hard(alpha_back, k_lambdaD=k_lambdaD))
    print("="*70)
...
def build_default_params(profile="matched"):
    ...
    profile="matched": keeps Chirag/Marco as matched as possible (good for apples-to-apples testing).
    profile="sheet_original": resets to the parameter values from your Google Sheet screenshot.
    '''
    if profile == "sheet_original":
        return {
            # --- Shared physical / plasma params (match sheet) ---
            "ne_m3": 2e11,
            "B_T": 3.60e-5,
            "Te_K": 500,
            "Ti_K": 500,

            # --- Chirag / ISR (match sheet) ---
            # Sheet lists theta=0 for ISR (your code uses theta for k||/k⊥ decomposition)
            "theta_deg": 0,

            # IMPORTANT: your Chirag implementation uses "lambda_r_m" as the wavelength feeding k = 2π/λ.
            # In your earlier comparisons you used 0.341 m (which corresponds to c/frad/2 for frad=440 MHz).
            # Keep 0.341 here to preserve the original “match-as-much-as-possible” behavior.
            "lambda_r_m": 0.341,

            # Collision freqs (match sheet)
            "nu_e_hz": 3.32424637,
            "nu_i_hz": 1364.30161374848,

            # Summation/control
            "n_terms": 2001,  # keep your high-order sum (what you used in the original compare script)
            # Ion species: O+ mass ~2.66e-26 kg, density=ne
            "ion_species": [{"mass": 2.66e-26, "fraction": 1.0, "density": 2e11}],

            # --- Marco / EE (match sheet) ---
            "ion_mass_amu": 16,
            "ion_comp": 1,

            # Marco spectral grid controls (match sheet)
            "marco_N": int(100E3),
            "marco_fs_hz": 12E6,
            "marco_frad_hz": 440E6,
            "marco_aspdeg": 90,   # sheet lists 90 deg
            "marco_modgy": 1,
            "marco_modnu": 2,

            # Matching knobs
            # For a true "sheet reset", keep Chirag collision freqs as the fixed sheet values
            # (i.e., do NOT overwrite them from Marco's collision model).
            "match_nu_to_marco": False,
        }

    # Default: "matched" profile (good for tests)
    return {
        "ne_m3": 2e11,
        "B_T": 3.60e-5,
        "Te_K": 500,
        ...
        "nu_i_hz": 1364.30161374848,
        "n_terms": 2001,
        # Use density=None so it always tracks ne_m3 in sweeps
        "ion_species": [{"mass": 2.65686e-26, "fraction": 1.0, "density": None}],
        # Marco defaults
        "ion_mass_amu": 16,
        "ion_comp": 1,
        ...
        "marco_modgy": 1,
        "marco_modnu": 2,
        # Use Marco collision model to keep both implementations consistent (unless you turn it off)
...
if __name__ == "__main__":
    ...
    p.add_argument("--profile", default="sheet_original", choices=["sheet_original", "matched"],
                   help="Parameter preset: sheet_original resets to your Google Sheet values; matched keeps apples-to-apples matching.")
    ...
    if args.test == "single":
        ...
        regime_ch = classify_thomson_hard(alpha_chirag_back)
        regime_m  = classify_thomson_hard(alpha_marco_back)
        ...
        plot_combined_spectra(
            ...
            (f_m/1e6, Sm),
            [params["Te_K"]],
            alpha_chirag=alpha_chirag_back,
            alpha_marco=alpha_marco_back,
        )
